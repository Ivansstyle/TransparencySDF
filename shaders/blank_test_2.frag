...
// Smart lerp function that doesn't work the way it supposed to _IS TODO: fix
...
mat2x3 sphere1, sphere2, pos;
    ...
    sphere1 = sdSphere(_pos1, 2.0f, vec4(1.0f,0.0f,1.0f,0.5f)); // red
    _pos1 = _position +      vec3(1.5f,0,0);
    sphere2 = sdSphere(_pos1, 2.0f, vec4(1.0f,1.0f,1.0f,1.0f)); // blue

    //pos = TopUnion(sphere1, sphere2);
    //pos = TopIntersection(sphere1, sphere2);
    pos = opBlend(sphere2, sphere1, 4.f);
...
TraceResult castRay(mat2x3 _ray)
{
 TraceResult trace; // Forward Tracing
  ...
  {
    mat2x3 r = map(_ray[0] + trace.t * _ray[1]);
    trace.d = r[0].x;
  ...
  {
    mat2x3 r = map(bray[0] + trace.bt * bray[1]); // create a ray from backwards ray to 
    trace.bd = r[0].x;
...
vec3 calcNormal(vec3 _position)
{
  ...
  vec3 normal = normalize(offset.xyy*map( _position + offset.xyy )[0].x +
                          offset.yyx*map( _position + offset.yyx )[0].x +
                          offset.yxy*map( _position + offset.yxy )[0].x +
                          offset.xxx*map( _position + offset.xxx )[0].x);
...
float calcAO(vec3 _position, vec3 _normal)
{
  ...
  for(int i = 0; i < 5; i++)
  {
    ...
    float dd = map(aopos)[0].x; // Map is here! (AO map)
    occ += -(dd-hr)*sca;
    sca *= 0.95;
  }
...
float softshadow(vec3 ro, vec3 rd, float mint, float tmax)
{
  ...
  for(int i = 0; i < 16; ++i)
  {
    float h = map(ro + normalize(rd)*t)[0].x; // Map is here!
    res = min(res, 8.0*h/t);
    t += clamp(h, 0.02, 0.10);
